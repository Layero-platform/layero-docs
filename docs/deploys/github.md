...
и production-адрес проекта начинает отдавать свежий артефакт.
...
`<org>-<project>-<branch>.preview.layero.ru`. Apex **не трогается** —
...
После первого `ready`-деплоя в default-ветке и **preview-URL ветки**, и
**production-адрес проекта** работают сразу — оба покрыты wildcard-сертификатом
своей зоны и не требуют регистрации хоста. Ожидания на 5–15 минут, которое было
во времена CDN перед сайтами, больше нет (см.
[Окружения, preview и production](./environments.md)).

Пока успешного деплоя нет вообще, на адресе показывается страница
**«Сайт скоро появится»** — чтобы зашаренная ссылка не отдавала 404.
