...
## Если IDE предложит «Authenticating…»

Закрой карточку кнопкой **Skip** — на подключение это не влияет. Codex, а за
ним и некоторые другие редакторы, при первом обращении к удалённому серверу
пробуют OAuth, ещё не зная, нужен ли он. У Layero его нет: сервер публичный
и на запросы отвечает без авторизации.

Токен, если он нужен, передавай заголовком, а не через этот диалог. Как
именно — в разделе [Подключение аккаунта](#подключение-аккаунта).
