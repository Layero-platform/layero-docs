...
:::warning[Токен деплоит всё, но необратимое ему закрыто]
Токен работает с любым вашим проектом — своего разделения по проектам у него
нет. Заводите отдельный токен на каждый репозиторий: тогда компрометацию
одного закрываете, не ломая остальные.

Необратимые операции токену без скоупа `admin` отказывают: удаление проекта и
аккаунта, переименование организации, приглашение и удаление участников,
отвязка карты и смена тарифа. То есть утёкший CI-токен выкатит чужой код, но
не сотрёт проект и не тронет деньги.

Скоупы ограничивают именно **CI-токен** (`layero_ci_*`). Сессия, которую даёт
`layero login`, — это обычный вход в аккаунт, и она может всё то же, что и
браузер, независимо от того, какие скоупы у неё записаны.
:::
