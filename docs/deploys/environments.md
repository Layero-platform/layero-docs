...
| Preview-адрес ветки | сразу после успешного билда ветки | **пока ветка существует и превью не вытеснено** | Preview ветки — для проверки и шеринга |
...
С 2 сентября 2026 у правила есть оговорка: проект раздаёт не больше пяти
превью одновременно — см. [Сколько превью работает
одновременно](#сколько-превью-работает-одновременно). Ветка при этом никуда не
девается, и ссылка возвращается одним деплоем.
:::

### Сколько превью работает одновременно

Проект раздаёт **не больше пяти превью одновременно**. Production-окружение в
это число не входит: сколько бы веток в проекте ни было, апекс работает всегда.

**Сборку это не ограничивает.** Шестая ветка собирается точно так же, как
первые пять: логи, история, карточка в панели — всё на месте. Ограничено
только число превью-адресов, которые отвечают сайтом одновременно.

Когда превью становится больше пяти, вытесняется то, **которое дольше всех не
деплоили**. Считается не возраст ветки: ветка, заведённая полгода назад и
задеплоенная сегодня, останется на месте, пока в проекте есть заброшенные
превью.

Вытесненное превью переходит в состояние «приостановлено»:

- его адрес отвечает страницей «Превью приостановлено», а не ошибкой;
- ветка, её сборки и история остаются на месте;
- в панели окружение сворачивается в раздел «Без активного деплоя».

**Чтобы вернуть превью, задеплойте эту ветку снова** — push в неё возвращает
адрес без пересборки чего-либо ещё. Место при этом уступает следующий
кандидат, то есть превью, которое дольше всех не деплоили.

:::tip[CLI и агенты узнают о вытеснении сразу]
`layero deploy` печатает предупреждение, если деплой занял чужой слот:

```
! Превью feature/old-checkout приостановлено — этот деплой занял его место
  Вернуть: задеплойте ту ветку снова или откройте панель
```

В JSON-режиме это событие `preview_evicted` со списком вытесненных превью;
оно приходит **до** `ready`, чтобы агент не счёл работу законченной раньше,
чем узнает о чужом адресе, переставшем отвечать.
:::

:::note[Почему лимит вообще есть]
Каждое живое превью рантайм-приложения держит на узле собственный образ.
Проект с сотней веток съедал узел целиком: 2 сентября 2026 один такой проект
держал 63% образов рантайм-узла по занятым байтам, и холодный старт чужих
сайтов доходил до полутора минут. Раньше ёмкость узла никто не считал заранее,
и нехватку обнаруживало переполнение. Лимит считает её заранее.
:::
