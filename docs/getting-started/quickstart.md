...
:::note[Кода ещё нет?]
Эта страница про публикацию **готового** проекта. Если сайта пока не
существует и хочется собрать его с нуля прямо в чате IDE — это другой
инструмент: [плагин `@layero`](../plugin/intro) для Cursor, Claude Code и
Codex. Описываете идею одной фразой, отвечаете на два-три коротких вопроса,
получаете готовую страницу и сразу публикуете.
:::
