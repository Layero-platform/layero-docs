...
:::warning `Cache-Control` сейчас приходит дважды
Проверено на живом проекте 27.07.2026. Если задать `Cache-Control` в
`_headers`, edge всё равно добавляет свой, и в ответе оказываются **два**
заголовка:

```
cache-control: public, max-age=31536000, immutable          ← ваш
cache-control: public, max-age=0, s-maxage=60, stale-while-revalidate=600
```

По стандарту такие поля объединяются в один список директив, и в нём
оказываются два разных `max-age`. Как поступит конкретный клиент или
промежуточный кэш — не определено.

Практический итог: **рассчитывать на «долгий» кэш ассетов через `_headers`
сейчас нельзя.** Остальные заголовки — включая security-заголовки, ради
которых файл обычно и заводят, — работают как описано: они добавляются, а не
конфликтуют. Проверено отдельно на всех трёх формах путей (`/*`, точный путь,
wildcard).
:::
