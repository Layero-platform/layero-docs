...
**Project → Settings → Передача проекта**: начните вводить название аккаунта
получателя — подсказка предложит подходящие. Заявка появится у получателя
**на дашборде**, блоком «Входящие передачи проектов»; он принимает или
отклоняет её там. Пока заявка висит, отправитель может снять её кнопкой
«Отменить запрос» в том же блоке настроек.

О заявке получателю приходит письмо. Если он отклонит её, письмо придёт уже
отправителю, а в настройках проекта появится отметка об отказе. Заявка, на
которую не ответили за 14 дней, истекает сама — до этого момента отправить
вторую нельзя, у проекта одновременно бывает только одна заявка.

Проект принадлежит ровно одному аккаунту, поэтому после переноса он пропадает
у отправителя: остаться в нём в какой-либо роли нельзя.

Адрес при переносе **не меняется** — ни бесплатный поддомен, ни подключённые
домены. Вместе с проектом едут переменные окружения, деплои, настройки
доступа и аналитика.

Если репозиторий проекта подключён к GitHub, а ни у кого в принимающем
аккаунте нет к нему доступа на запись, связь с репозиторием отметится как
разорванная — деплой будет заблокирован, пока её не восстановят на странице
проекта.
