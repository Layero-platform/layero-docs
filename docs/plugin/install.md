...
Часть редакторов при подключении удалённого сервера сначала пробует OAuth —
просто на всякий случай, ещё не зная, нужен ли он. У Layero его нет: сервер
публичный и кода 401 не возвращает никогда. Карточку авторизации можно
спокойно закрыть кнопкой **Skip** — подключение от этого не пострадает.

Токен, если он нужен, передаётся заголовком, а не через этот диалог. Как
именно — в следующем разделе.
