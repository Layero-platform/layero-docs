...
У проекта **один стабильный production-адрес** — apex. Что именно отдаёт apex — указывает **production-pointer**, который двигается автоматически (push в default-ветку **или** прямая `layero deploy`-загрузка в проект без репозитория — она авто-промоутится в apex) либо вручную (промоут из UI/CLI). Именованные ветки получают **временные preview-ссылки** на 24 часа и apex не трогают — но создаются они пушем в репозиторий, а не флагом CLI: `layero deploy --branch` игнорируется, архивные загрузки всегда идут в окружение `cli` (см. [`layero deploy`](../cli/deploy.md)).
