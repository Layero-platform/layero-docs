...
⚠️ **Кроме выделенного инстанса.** Там сертификат самоподписанный, проверить
подлинность сервера нечем, и платформа сама выдаёт строку с `require` —
какой бы режим ни стоял в настройках базы. Канал при этом шифруется;
защиты от подмены сервера у выделенного инстанса нет. Панель говорит об этом
прямо в окне подключения, а переключатель там погашен.
