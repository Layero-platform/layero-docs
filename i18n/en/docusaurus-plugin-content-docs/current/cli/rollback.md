...
2. После confirm бэкенд:
   ...
   * Сбрасывает кеш резолвера (новые запросы получат старый артефакт;
     ранее закэшированные ответы обновляются по короткому TTL).
   * Возвращает обновлённый deploy с новым активным флагом.

3. Через несколько секунд (кеш на edge — до минуты) адрес отдаёт прошлую версию.
